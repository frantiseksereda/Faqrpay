...
class TransactionManager(
    ...
    suspend fun createTransactionAndQrCode(amount: Double, currency: String): Pair<String, Bitmap>? {
        return withContext(Dispatchers.IO) {
            try {
                // 1. Create record in DB and get new transactionId
                val transactionId = transactionRepo.addTransaction(amount, currency)

                // 2. Get IBAN from settings
                val iban = settingsRepo.getIban() // Assumes this returns valid IBAN string

                // 3. Create SPD (Short Payment Descriptor) string
                ...
                // 4 & 5. Generate and return QR code bitmap
